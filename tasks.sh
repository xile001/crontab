...
subtask(){
  ...
  local pid=${1##*p}
  #有序集合
  for key in `$REDISEXEC zrange ${REDIS_KEYS}z${pid} 0 -1`
  do
    ...
    if [[ $flag -eq $run_start && $run_status -eq 1 && -f $files ]];then
      ...
      case $params in
        0)
          params="tid:s$ids tinterval:s$interval tgroup_name:$group tclassify:$classify tname:$name"
          ;;
        *)
          params="$params tid:s$ids tinterval:s$interval tgroup_name:$group tclassify:$classify tname:$name"
      esac
      case $suffix in
        php)
...
while true
do
  for keys in `$REDISEXEC keys ${REDIS_KEYS}p*`
  do
    #当前时间
    now_time=`date "+%s"`
    ...
    case $classify in
      ...
      2)
        ...
        if [[ $month != ${now_date_arr[1]} && $month != '00' ]];then
          state=0
        elif [[ $state -gt 0 && $month == ${now_date_arr[1]} ]];then
          state=1
          interval="${interval}m$month"
        elif [[ $state -gt 0 && $month == '00' ]];then
          state=$state
        else
          state=2
        fi
        ...
        if [[ $week != ${now_date_arr[2]} && $week != '0' ]];then
          state=0
        elif [[ $state -gt 0 && $week == ${now_date_arr[2]} ]];then
          state=1
          interval="${interval}w$week"
        elif [[ $state -gt 0 && $week == '0' ]];then
           state=$state
        else
          state=2
        fi
        ...
        if [[ $day != ${now_date_arr[3]} && $day != '00' ]];then
          state=0
        elif [[ $state -gt 0 && $day == ${now_date_arr[3]} ]];then
          state=1
          interval="${interval}d$day"
        elif [[ $state -gt 0 && $day == '00' ]];then
          state=$state
        else
          state=2
        fi
        ...
        if [[ $hour != ${now_date_arr[4]} && $hour != '00' ]];then
          state=0
        elif [[ $state -gt 0 && $hour == ${now_date_arr[4]} ]];then
          state=1
          interval="${interval}h$hour"
        elif [[ $state -gt 0 && $hour == '00' ]];then
          state=$state
        else
          state=2
        fi
        ...
        if [[ $minute != ${now_date_arr[5]} && $minute != '00' ]];then
          state=0
        elif [[ $state -gt 0 && $minute == ${now_date_arr[5]} ]];then
          state=1
          interval="${interval}i$minute"
        elif [[ $state -gt 0 && $minute == '00' ]];then
          state=$state
        else
          state=2
        fi
        ...
        if [[ $second != ${now_date_arr[6]} && $second != '00' ]];then
          state=0
        elif [[ $state -gt 0 && $second == ${now_date_arr[6]} ]];then
          state=1
    ...
    if [[ $int_runtime -le $now_time && $state -eq 1 && $flag -eq $run_start && $run_status -eq 1 && -f $files ]];then
      num=$[num + 1]
      $REDISEXEC hmset $keys start "$now_time" nums "$num" flag "$run_stop" &>/dev/null
      case $params in
        0)
          params="tid:p$ids tinterval:$interval tgroup_name:$group tclassify:$classify tname:$name"
          ;;
        *)
          params="$params tid:p$ids tinterval:$interval tgroup_name:$group tclassify:$classify tname:$name"
      esac
      case $suffix in
        php)
